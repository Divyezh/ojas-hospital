...
async function loadFontFromStylesheet(url: string): Promise<string> {
  if (!url) {
    console.warn('CircularGallery: loadFontFromStylesheet called with empty URL');
    return 'sans-serif';
  }
...
async function resolveFont(font: string, fontUrl?: string): Promise<string> {
  // Only fetch a remote font if the caller explicitly provides a fontUrl.
  // Never auto-fetch — that triggers CSP violations in strict environments.
  if (!fontUrl) {
...
class App {
  ...
  boundOnTouchUp!: () => void; boundOnKeyDown!: (e: KeyboardEvent) => void;
...
export default function CircularGallery({
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Department gallery. Use Left and Right Arrow keys to navigate."
    />
  );
}

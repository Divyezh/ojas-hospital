...
export const Card = React.forwardRef<HTMLDivElement, CardProps>(
  ({ className, glass = false, hoverLift = true, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn(
          'rounded-3xl border border-slate-200/80 bg-white p-6 transition-all duration-300 shadow-soft-sm',
          glass && 'glass-card border-white/60',
          hoverLift && 'hover:-translate-y-1 hover:shadow-soft-lg hover:border-maroon-600/30',
          className
        )}
        {...props}
      />
